...
class UploadController extends Controller
{
    ...
    public function edit(Request $request) {
        ...
        $existingModel = Item::find($idItem);
...
//        $item = new Item();
        $existingModel->img = $path; // Store the file path in your item model
        $existingModel->name = $request->input('name');
        $existingModel->desc = $request->input('desc');
        $existingModel->defects = $request->input('defects');
        $existingModel->amount = $request->input('amount');

        // Find the Type by name and set the type_id
        $type = ItemType::firstOrCreate(['name' => $request->input('types')]);
        $existingModel->type_id = $type->id;

        // Find the Condition by name and set the condition_id
        $condition = Condition::firstOrCreate(['name' => $request->input('conditions')]);
        $existingModel->condition_id = $condition->id;

        $existingModel->save();

        return redirect()->intended('/dashboard')->with('success', 'Berhasil Edit');
    }
}
